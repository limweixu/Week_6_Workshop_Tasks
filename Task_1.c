...
void filter_ascending(char* string_1)
{
    for (int i = 1; i < strlen(string_1); i++)//number of passes
    {
        ...
        for (int j = 0; string_1[j+2] != '\0'; j++)//number of comparisons
        {

            if (string_1[j] > string_1[j+1]) //comparing ascii value of current to adjacent
            {
                for (int k = 0; string_1[k] != '\0'; k++)
                {
                    // printf("k = ",k);
                    string_1[k] = string_1[k+1];//to delete the unwanted
                    // printf(string_1);
                }
            }
        }
